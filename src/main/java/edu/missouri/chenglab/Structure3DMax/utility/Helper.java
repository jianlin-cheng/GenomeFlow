...
public class Helper {	
  ...
  /**
   *  Detect if Matrix or List
   * @param fileName
   * @return
   * @throws Exception
   */
	
	public int inputdetector(InputParameters_3DMax inputPara) throws Exception{
		
		
		   String fileName = inputPara.getInput_file();
		   Scanner input = new Scanner (new File( fileName));
		   String sep = "\\s+" ;
		   int count =  0; //read the first 15 lines to detect 
		   
		   int MatrixFreq = 0; int TupleFreq = 0;
			while(input.hasNextLine() && count < 15)
			{	
				String [] line = null;
			...
			if (MatrixFreq  > TupleFreq) {
				return 1;   // 1 Signifies Input is a Matrix
			}
			
			
			return  0;      // 0 Signifies Input is a Tuple
	 ...
	 */
	public String readMatrixtoList(InputParameters_3DMax inputPara) throws Exception{		
			
		String Filename = inputPara.getInput_file();
		String sep = "\\s+" ; 	
		String newFile = inputPara.getOutput_folder() + "/InputData_in_TupleFormat.txt" ;
		...
		FileWriter writer = new FileWriter(newFile, true);
		 ...
		 System.out.println(String.format("Conversion to List Successful, New File = %s", newFile));
		 writer.close();
	 ...
	 * @throws Exception
	 */
	public List<Constraint> readContactList(InputParameters_3DMax inputPara, List<Integer> lstPos,double...thres) throws Exception{
