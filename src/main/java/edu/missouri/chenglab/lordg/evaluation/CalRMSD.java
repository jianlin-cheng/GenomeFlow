...
public class CalRMSD {
	...
	public static double correlationIFvsDist(double[] str, List<Constraint> lstCon, int interval) throws Exception{		
		...
		for(Constraint con:lstCon){
			...
			i = con.getPos1();
			j = con.getPos2();
			if (i != j && Math.abs(i - j) >= interval && con.getIF() > 0){
	...
	public static double rmse(double[] str, List<Constraint> lstCon) throws Exception{
		double d,sum = 0.0;
		int i,j;
		//PrintWriter pw = new PrintWriter("distance_err1.txt");
		for(Constraint con:lstCon){
			i = con.getPos1();
			j = con.getPos2();
			d = helper.calEuclidianDist(str[i * 3], str[i * 3 + 1], str[i * 3 + 2], str[j * 3], str[j * 3 + 1], str[j * 3 + 2]);
			
			//pw.println(con.getPos1() + "\t" + con.getPos2() + "\t" + con.getIF() + "\t" + con.getDist() + "\t" + d + "\t" + Math.abs(con.getDist() - d));
