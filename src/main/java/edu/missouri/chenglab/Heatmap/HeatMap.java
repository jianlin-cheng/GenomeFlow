...
public class HeatMap extends JPanel
{
    ...
    //Tosin
    private boolean runClusterTAD = false;
    private boolean drawshowallTAD = false;
    private boolean drawselectTAD = false;
     ...
     * @param draw TAD on Heatmap
     */
    public void setselectTAD(boolean drawselectTAD)
    {
        this.drawselectTAD = drawselectTAD;
    ...
    /**
     * Updates the status text. Calls repaint() when finished.
     * @param The new status specified
     */
    ...
    private void drawData()
    {
        ...
        // allocate value to GraphicsOnly
        int w = bufferedImage.getWidth();
        int h = bufferedImage.getHeight();
        int scale = getscale(w);
        w*=scale;
        h*=scale;
        Image =ScaleIt(bufferedImage,w,h) ; // returns and image    
        
        app = new GraphicsOnly(Image); // Initialize the value
    }
    
    
    
    /**
     * To scale the value of buffered image
     * @param imageToScale
     * @param dWidth
     * @param dHeight
     * @return
    ...
    public static BufferedImage ScaleIt(BufferedImage imageToScale, int dWidth, int dHeight) {
    ...
    /**
     * Returns the best sclae based on HeatMap size
     * @param width
     * @return
     */
    ...
    public static int getscale(int width) {
    	int out =  1;
    	if(width < 50) {	out = 15; 	}
    	else if(width < 100) {	out = 10; 	}
    	else if(width < 250) {	out = 5; 	}
    	else if(width < 400) {	out = 3; 	}
    	else {out = 1; };
     ...
     */
    private void updateDataColors_ext(int [][] TADindex)
    {
    	int boundarycolor = 0;
        //We need to find the range of the data values,
        // in order to assign proper colors.
        ...
        for (int i = 0; i < TADindex.length;i++) {
        	int start = TADindex[i][0]; int limit= TADindex[i][1];	
        	int x1 = TADindex[i][0]; int x2 = TADindex[i][1]; 
        	// horizontal
             for (int y = start; y <= limit; y++)
             {
                 dataColorIndices[x1][y] = boundarycolor; //top
                 dataColorIndices[x2][y] = boundarycolor; //bottom
             }  
             //vertical      
         	int y1 = TADindex[i][0]; int y2 = TADindex[i][1];
              for (int x = start; x <= limit; x++)
              {
                  dataColorIndices[x][y1] = boundarycolor; //top
    ...
    public void updateData_ext(double[][] data, int [][] TADindex, boolean useGraphicsYAxis)
    {
        this.data = new double[data.length][data[0].length];
        for (int ix = 0; ix < data.length; ix++)
        {
